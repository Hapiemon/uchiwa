...
export function AnniversaryCard({
  ...
  const { daysUntil, daysSince, nextMilestone } = useMemo(() => {
    ...
    const milestones = [
      7, 30, 50, 100, 150, 200, 300, 365, 500, 1000, 1500, 2000, 3000, 5000,
      10000,
    ];
    ...
    return {
      ...
      daysSince: daysSinceAnniversary,
      nextMilestone:
        nextMilestoneValue && daysToMilestone
    ...
    <div className="bg-gradient-to-br from-pastel-pink to-pastel-purple p-6 rounded-2xl shadow-md text-white">
      ...
      {getRepeatLabel() && (
        <p className="text-sm opacity-90 mb-2">
          🔄 {getRepeatLabel()}
          {anniversary.repeatInterval === "WEEKLY" &&
            ` ${new Date(anniversary.date).toLocaleDateString("ja-JP", {
              weekday: "long",
            })}`}
          {anniversary.repeatInterval === "MONTHLY" &&
            ` ${new Date(anniversary.date).getDate()}日`}
          {anniversary.repeatInterval === "YEARLY" &&
            ` ${new Date(anniversary.date).toLocaleDateString("ja-JP", {
              month: "long",
              day: "numeric",
            })}`}
